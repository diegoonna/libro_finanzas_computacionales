...
nocite: |
  @lopezdeprado2018, @bailey2014, @harvey2015, @white2000
---
...
Todo el capítulo trabajó sobre ruido fabricado, donde sabíamos la respuesta de antemano. El examen final es con datos donde **nadie** sabe la respuesta: corremos el walk-forward completo sobre cinco años del SPY, del dataset del libro (`datos/precios.csv`, ver @sec-datos-financieros).

```{python}
#| label: wf-datos-reales
precios = pd.read_csv("../datos/precios.csv", index_col="Date", parse_dates=True)
r_spy = precios["SPY"].pct_change().dropna().values

oos_spy, elec_spy = walk_forward(r_spy, n_train=504, n_test=126, ventanas=range(2, 62))

print(f"Tramos operados: {len(elec_spy)} | Ventanas elegidas: {elec_spy}")
evaluar(oos_spy, "Momentum SPY (walk-forward)")
evaluar(r_spy[-len(oos_spy):], "Buy & hold SPY (mismo período)")

_ = reporte(oos_spy, "Momentum SPY (OOS)")
print()
_ = reporte(r_spy[-len(oos_spy):], "Buy & hold SPY")
```

Este resultado es una clase entera de honestidad estadística, así que leelo despacio:

- **Las ventanas elegidas son estables entre tramos** — nada del salto errático que vimos sobre ruido puro. Eso sugiere que el momentum del índice tiene *algo* de estructura real.
- El Sharpe out-of-sample es positivo, pero el **p-value no baja del umbral**: con ~2.5 años de track record OOS, la fórmula $t \approx \text{Sharpe} \times \sqrt{\text{años}}$ hace su trabajo implacable. Un Sharpe cercano a 1 con dos años y medio de datos **no es evidencia suficiente** — verificalo vos mismo con la fórmula.
- Y la comparación que un backtest siempre debe incluir: en el mismo período, **comprar y no hacer nada rindió más**. El período OOS fue mayormente alcista, y toda estrategia que pasa tiempo en cash paga ese costo. El consuelo del momentum es un drawdown máximo menor — es una estrategia *defensiva*, y su valor aparecería en un bear market largo, no en este tramo. Elegir el benchmark correcto es parte del protocolo.

Ah, y esto es *antes* de costos de transacción. La conclusión metodológica —no sobre el momentum sino sobre el oficio— es que un backtest honesto casi nunca da un veredicto rotundo: da matices, condiciones y un p-value incómodo. Desconfiá de los que no.
...
5. **Tu propio protocolo**: elegí una regla simple (cruce de medias móviles, por ejemplo), escribí la hipótesis económica ANTES de correr nada, y ejecutá el protocolo completo de 7 pasos sobre alguna de las acciones del dataset del libro (`datos/precios.csv`). Documentá cuántas variantes probaste.
:::
